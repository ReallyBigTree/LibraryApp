...
public class LibFrame extends JFrame {
	...
	JLabel lblISBN;
	JLabel lblISBNicon;
	URL defaultImageURL= new URL("https://s.gr-assets.com/assets/nophoto/book/111x148-bcc042a9c91a29c1d680899eff700a03.png");	
	JPanel panel; 
	...
	public LibFrame() throws IOException {
		...
		btnSearchISBN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {				
				...
				/*
				//this section of the event will update the book panel with other books by the same author. 
				//list which we can remove found books from without changing the global list. 
				ArrayList<Book> sublist = new ArrayList<Book>();
				try {
					sublist = LibraryApp.createBookList();
				} catch (IOException e1) {		
					e1.printStackTrace();
				}
				InsertionSort sorter = new InsertionSort();
				sorter.sortByAuthorAscending(sublist);
				
			
				//Search for other books by same author, make sure the books are found and that the title is that the same
				// as the isbn searched.
				BinarySearch search = new BinarySearch();
				int index = BinarySearch.binarySearch(sublist, book.author);
				
				if(index != -1 && sublist.get(index).title != book.title) {
					bookPanel3_1.refreshPanel(sublist.get(index));
					sublist.remove(index);
					index = BinarySearch.binarySearch(sublist, book.author);
					if(index != -1) {
						bookPanel3_1_1.refreshPanel(sublist.get(index));
					}
				}
				else {
					//what to do if book not found // set rtandom? popular? static?
				*/	
	    ...
	    //In the combo box item state changed event we will update the book panels with the sorted results - this will probably get ugly.
	    String[] comboBoxOptions = {"Recommended", "Title A-Z", "Author A-Z", "Ratings High", "Ratings Low","Year Low","Year High"};
			...
			private void getComboBoxSelection(JComboBox comboBox, ItemEvent e) {
				if(e.getStateChange() == ItemEvent.SELECTED) {	    
	    			if(comboBox.getSelectedIndex() == 0) {    				
	    				System.out.println("sort()   Recommended"); //maybe this function could find books by the same author or something 
	    				InsertionSort sorter = new InsertionSort();
	    			...
	    			if(comboBox.getSelectedIndex() == 4) {
	    				...
	    				sublist = sorter.getNormalizedRatingsList(bookList);
	    				//wee need to find the indices where the -1 ratings are gone, and the URL is okay..maybe regex "http" 
	    				updateBookPanels(sublist.get(0), sublist.get(1), sublist.get(2));
	    			}
